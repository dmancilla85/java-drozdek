...
import org.drozdek.lists.iterators.DoublyLinkedListIterator;
...
public class DoublyLinkedList<T> implements Iterable<T>, ListInterface<T> {
    ...
    public void addToTail(T data) {
        if (!isEmpty()) {
            tail = new DoublyLinkedListNode<>(data, null, tail);
            tail.getPrevious().setNext(tail);
        } else head = tail = new DoublyLinkedListNode<>(data);
    }
    ...
    private void removeMiddleNode(T data) {
        DoublyLinkedListNode<T> predecessor = head;
    ...
    public DoublyLinkedListNode<T> viewHeadNode() {
        return head;
    }
    ...
    /// Note: This method exposes the internal node structure. Use with caution
    /// as it allows direct manipulation of the list's internal structure.
    public DoublyLinkedListNode<T> viewTailNode() {
        return tail;
    }
}
